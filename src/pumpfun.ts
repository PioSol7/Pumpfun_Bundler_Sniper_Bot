...
export class PumpFunSDK {
  ...
  async getBuyIxsBySolAmount(
    ...
    mint: PublicKey,
    buyAmountSol: bigint,
    slippageBasisPoints: bigint = BigInt(10000),
    commitment: Commitment = DEFAULT_COMMITMENT
  ) {
    let bondingCurveAccount = await this.getBondingCurveAccount(
      global_mint,
  ...
  async getTokenPrice(mint: PublicKey) {
    ...
    return Number(bondingCurveAccount.virtualSolReserves + bondingCurveAccount.realSolReserves) / Number(bondingCurveAccount.virtualTokenReserves + bondingCurveAccount.realTokenReserves)
  }
